...
class PhotoPal:
    ...
    def _prettify_suggestion_string(self, *, suggestion: dict, container: str) -> str:
        per_mille = (
            100
            * (suggestion["images"] + suggestion["videos"])
            / self.overall_file_counter
        )
        return (
    ...
    def _get_sorted_map_for_directories(self, *, reverse: bool):  # noqa: PBR002
        # Collect data
        ...
        for key, container_dict in self.dir_map.items():
            sorted_map[key] = sorted(
                container_dict, key=lambda item: item["total"], reverse=reverse
            )
    ...
    def _print_low_hanging_fruits(self, *, sorted_map: dict):
        ...
        for fruit in low_fruit_list:
            print(
                f'"{fruit["container_name"].split(" ")[1]}/{fruit["name"]}" '
    ...
    def _print_challenges(self, *, sorted_map: dict):
        ...
        challenge_list = sorted(
            challenge_list, key=lambda item: item["total"], reverse=True
        )

        print("\n*Herausforderungen*")
        for challenge in challenge_list:
            print(
                f'"{challenge["container_name"].split(" ")[1]}/{challenge["name"]}" '
    ...
    def process(self):
        ...
        # Print challenges
        self._print_challenges(
            sorted_map=self._get_sorted_map_for_directories(reverse=True)
        )
        ...
        for key, container_dict in sorted_map_asc.items():
            suggestion_list.append(
                self._prettify_suggestion_string(
                    suggestion=self._pick_suggestion(container=container_dict),
                    container=key,
                )
            )
        self._calculate_suggestion_container(suggestion_list=suggestion_list)
