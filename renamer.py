...
class Renamer:
    ...
    PATTERN_FOTO = (
        r"Foto\s(\d{2}).(\d{2}).(\d{2}),\s(\d{2})\s(\d{2})\s(\d{2})[\w\(\))]*%s$"
    )
    ...
    def process(self, *, name_pattern: str):
        ...
        for element in os.scandir(Path(self.FOTO_DIR)):
            if element.is_file():
                ...
                hour = (
                    matches[4]
                    if matches.lastindex is not None and matches.lastindex >= 4
                    else "12"
                )
                minute = (
                    matches[5]
                    if matches.lastindex is not None and matches.lastindex >= 5
                    else "00"
                )
                second = (
                    matches[6]
                    if matches.lastindex is not None and matches.lastindex >= 6
                    else "00"
                )
                new_filename = (
                    f"{year}-{month}-{day} {hour}.{minute}.{second}{extension}"
                )
